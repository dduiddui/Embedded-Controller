...
void USART_setting(USART_TypeDef* USARTx, PinName_t TX, PinName_t RX, uint32_t baud){
//1. GPIO Pin for TX and RX	
  ...
  unsigned int pinTX;
  ecPinmap(TX, &GPIO_TX, &pinTX);
	
	GPIO_TypeDef *GPIO_RX;
  unsigned int pinRX;
  ecPinmap(RX, &GPIO_RX, &pinRX); 
	
	
	GPIO_init(TX, AF);
	GPIO_otype(TX, EC_PUSH_PULL);
	GPIO_pupd(TX, EC_NONE);		
	GPIO_ospeed(TX, EC_HIGH);						
	
	GPIO_init(RX, AF);
	GPIO_otype(RX, EC_PUSH_PULL);
	GPIO_pupd(RX, EC_NONE);
	GPIO_ospeed(RX, EC_HIGH);
