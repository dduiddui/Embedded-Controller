...
void TIM_period_us(TIM_TypeDef *TIMx, uint32_t usec){   
	...
	if (TIMx == TIM2 || TIMx == TIM5){
		...
		ARRval = Sys_CLK/PSCval*0.000001 * usec;		// 1MHz*usec
		TIMx->PSC = PSCval - 1;
		TIMx->ARR = ARRval - 1;
	}
	else{
...
void TIM_period_ms(TIM_TypeDef* TIMx, uint32_t msec){ 
	...
	//	uint16_t ARRval = (8400/(PSCval)*msec);  			// 84MHz/1000ms
	uint16_t PSCval;
	uint32_t Sys_CLK;
	...
	if (TIMx == TIM2 || TIMx == TIM5){
		...
		TIMx->PSC = PSCval - 1; // 840 or 160
		TIMx->ARR = ARRval - 1; // 100*msec if 20 period = 2msec
	}
	else{
		uint16_t ARRval;

		PSCval = Sys_CLK/100000;								// 840 or 160	--> f_cnt = 10kHz
		ARRval = Sys_CLK/PSCval/1000 * msec;		// 100kHz*msec
		TIMx->PSC = PSCval - 1;
		TIMx->ARR = ARRval - 1;
	}
...
// 3. NVIC Setting
	...
	NVIC_SetPriority(IRQn_reg,3);  // TIM priority 3
}
...
// TIMx->CCER &= ~(0b1010 << 4*(ICn-1));
	TIMx->CCER &= ~(TIM_CCER_CC1NP | TIM_CCER_CC1P);					// CCy(Rising) for ICn
